...
public class QuorumPeerConfig {
	...
	public void parse(String path) throws ConfigException {
		...
		try {
			...
			// as static configuration params
	...
	public static void writeDynamicConfig(String dynamicConfigFilename, String configFileStr,
		...
		// the following is for users who run without a dynamic config file (old
		// config file)
		// if the configuration changes (reconfiguration executes), we create a
		// dynamic config
		// file, remove all the dynamic definitions from the config file and add
		// a pointer
		// to the config file. The dynamic config file's name will be the same
		// as the config file's
		...
		if (configBackwardCompatibilityMode) {
			...
			try {
				...
				if (!tmpFile.renameTo(configFile)) {
					throw new IOException("renaming " + tmpFile.toString() + " to " + configFile.toString()
							+ " failed!");
				}
			} finally {
				if (out != null) {
	...
	/**
	 * Parse dynamic configuration file and return quorumVerifier for new
	 * configuration.
	 * 
	 * @param zkProp
	 *            Properties to parse from.
		...
		if (numMembers > 1 || (!standaloneEnabled && numMembers > 0)) {
			...
			if (clientPortAddress != null && qs != null && qs.clientAddr != null) {
				if ((!clientPortAddress.getAddress().isAnyLocalAddress() && !clientPortAddress.equals(qs.clientAddr))
						|| (clientPortAddress.getAddress().isAnyLocalAddress() && clientPortAddress.getPort() != qs.clientAddr
								.getPort()))
